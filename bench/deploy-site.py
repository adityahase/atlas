...
# files block, on both `listen 80;` and `listen [::]:80;`), `nginx -t`, then
# `systemctl reload nginx`. No Frappe/gunicorn boot, no process restart — sub-second.
# bench-cli emits the v6 listener itself (the edge proxy reaches the VM over its
# public /128 only), so there is no per-clone nginx surgery here.
#
...
# to the FQDN before `bench setup nginx` regenerates the admin vhost.
...
class DeploySiteInputs:
	...
	`mode` picks what the FQDN maps to (mirrors build.sh's bake mode):
	  * site  — rename the baked `site.local` dir to the FQDN, so the FQDN serves
	            the baked site.
	  * admin — set `[admin].domain = <fqdn>` so the FQDN serves the admin app
	            (no site rename; the golden carries no baked site in admin mode).
	Both then run `bench setup nginx`, which regenerates nginx to map either
	correctly (bench-cli emits the v6 listeners itself at the pinned commit).
...
def _setup_nginx_for_fqdn(fqdn: str) -> None:
	...
	`server_name <fqdn>` (matching the proxy's forwarded Host — the rename means the
	on-disk name == the Host, so no `default_server` catch-all is needed) and a
	`root .../sites/<fqdn>/public` files block, on both `listen 80;` AND `listen
	[::]:80;` (the pinned bench-cli emits the v6 listener itself — the edge proxy
	reaches the VM over its public /128, so v6 is the path that matters). It then
	`nginx -t`s and `systemctl reload`s — no Frappe boot, no process restart.
	...
	a dangling `server_name site.local`. Remove it before regenerating."""
...
def _set_admin_domain(fqdn: str) -> None:
	"""Admin mode: point the admin vhost at the FQDN, then regenerate nginx.

	With `[admin].domain = <fqdn>` set, `bench setup nginx` emits an `_admin.conf`
	vhost (`server_name <fqdn>`, `listen 80;` + `listen [::]:80;`) proxying to the
	socket-activated admin gunicorn — so the FQDN maps to the admin URL. We rewrite
	the committed bench.toml's `domain = ""` line in place (a plain text edit — no
...
def main() -> None:
	...
	→ map the FQDN (rename the site dir, or set [admin].domain) → `bench setup nginx`
	(regenerate the vhost + reload, v6 listener included by bench-cli) → local serving
	probe. No `set-admin-password` (the baked throwaway is rotated out of band), no
	`setup production`, no restart of gunicorn/supervisor — that absence is the
	latency win. Every bench command runs as the `frappe` user (the bake user).
	...
	if inputs.mode == "admin":
		log("admin mode: pointing [admin].domain at the FQDN + setup nginx …")
		...
		log("regenerating nginx vhost (setup nginx) …")
