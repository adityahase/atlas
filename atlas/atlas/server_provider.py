"""Server provisioning logic.

Separated from the doctype controller so the e2e and tests can call it
directly with a Provider doc, and so the controller stays thin.
"""

import frappe

from atlas.atlas.digitalocean import DigitalOceanClient, public_ipv4, public_ipv6
from atlas.atlas.networking import carve_virtual_machine_range
from atlas.atlas.secrets import get_secret
from atlas.atlas.ssh import connection_for_server  # noqa: F401  re-exported


def provision_server(provider, server_name: str) -> str:
	"""Create a droplet, insert a Server row, enqueue bootstrap."""
	if frappe.db.exists("Server", server_name):
		frappe.throw(f"Server {server_name} already exists")

	token = get_secret("Server Provider", provider.name, "api_token")
	client = DigitalOceanClient(token=token)
	droplet = client.create_droplet(
		name=server_name,
		region=provider.default_region,
		size=provider.default_size,
		image=provider.default_image,
		ssh_key_ids=[provider.ssh_key_id],
		tags=["atlas", server_name],
		ipv6=True,
	)

	server = frappe.get_doc({
		"doctype": "Server",
		"server_name": server_name,
		"provider": provider.name,
		"provider_resource_id": str(droplet["id"]),
		"region": provider.default_region,
		"size": provider.default_size,
		"status": "Pending",
	}).insert(ignore_permissions=True)
	frappe.db.commit()

	frappe.enqueue(
		"atlas.atlas.server_provider.finish_provisioning",
		queue="long",
		timeout=1800,
		server_name=server.name,
		droplet_id=droplet["id"],
	)
	return server.name


def finish_provisioning(server_name: str, droplet_id: int) -> None:
	"""Background job: wait for droplet active, record addresses, bootstrap."""
	server = frappe.get_doc("Server", server_name)
	provider = frappe.get_doc("Server Provider", server.provider)
	token = get_secret("Server Provider", provider.name, "api_token")
	client = DigitalOceanClient(token=token)

	droplet = client.wait_for_active(droplet_id, timeout_seconds=600)
	host_v4 = public_ipv4(droplet)
	host_v6, prefix_v6 = public_ipv6(droplet)

	server.ipv4_address = host_v4
	server.ipv6_address = host_v6
	server.ipv6_prefix = prefix_v6
	server.ipv6_virtual_machine_range = carve_virtual_machine_range(prefix_v6)
	server.status = "Bootstrapping"
	server.save(ignore_permissions=True)
	frappe.db.commit()

	try:
		server.bootstrap()
	except Exception:
		server.reload()
		server.status = "Broken"
		server.save(ignore_permissions=True)
		frappe.db.commit()
		raise

	server.reload()
	server.status = "Active"
	server.save(ignore_permissions=True)
	frappe.db.commit()
